...
export function formatMonitorResultEmail(
  ...
  // Prefer a local file in your repo; fall back to /mnt/data if you’ve mounted it.
  const logoPath =
  ...
  const baseUrl =
    `https://${process.env.REPLIT_DEV_DOMAIN || (process.env.REPLIT_DOMAINS?.split(',')[0]) || 'your-app.replit.app'}`;

  const reportHref = `${baseUrl}${conversationId ? `?conversation=${encodeURIComponent(conversationId)}` : ''}`;
...
<head>
  ...
  <title>${subject}</title>
  <style>
    /* Basic resets for better email-client consistency */
    ...
    /* Header with teal background and white circle logo */
    .header {
    ...
    /* Dark mode friendly tweaks (where supported) */
        ...
        <div class="container">
          ...
          <div class="header">
            <div class="logo-circle">
              <img src="cid:wyshbone-logo" alt="Wyshbone AI" width="72" height="72" style="width:72px; height:72px;" />
            </div>
