...
- **Trust-Neutral Result Language:** RunResultBubble uses neutral, factual language that avoids implying system failure. Key rules: (1) No arbitrary lead splitting — `splitLeadsByVerification` only uses actual `lead_verification` data or `verifiedExact >= allLeads` count; never slices by position. (2) Section headers say "Results (N)" or "Other results (N)" instead of "Candidates (not fully verified)". (3) Summary text for PARTIAL/STOP without a user-specified target count uses neutral framing ("I found N results for your search") instead of failure language. (4) Unverified leads show no badge (null) instead of "Candidate". (5) Unverifiable attribute badge says "X not checked" instead of "Not confirmed: X". (6) Next action buttons use exploratory framing ("Search more results", "Try a different search") with context-aware heading ("Next steps" when target count specified, "Explore further" otherwise). (7) STATUS_CONFIG labels: PARTIAL="Results ready", STOP="Search complete".
