...
- **RunResultBubble (Feb 2026):** Rich result bubble (`client/src/components/results/RunResultBubble.tsx`) rendered inline in chat for RUN lane delivery_summary artefacts. Deterministic templates (no LLM): PASS shows "I found N results that match", PARTIAL shows shortfall, STOP with unverifiable attribute shows "couldn't reliably confirm {attr}", STOP with none shows "couldn't find any". Lead rows show name, area, website link, Google Maps link (from place_id), and phone (only if verified). Badges: "Verified match" (only when verified_exact_count > 0), "Candidate", or "{attr}: unknown". Next action buttons prefill chat input. Never shows "Exact matches" label unless verified_exact_count > 0; unverified leads in delivered_exact render as "Candidates (not fully verified)".
