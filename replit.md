...
- **Demo Orchestrator (`DemoOrchestratorPanel`):** Component (`client/src/components/DemoOrchestratorPanel.tsx`) with 3 preset demo buttons: Clean success, Partial delivery, Intelligent stop. Each triggers `POST /api/demo/{case}` and surfaces the resulting run. Shows only button + status, no internal artefacts.
