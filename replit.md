...
- **Supervisor Context System:** An intelligent personalization layer (`server/lib/context.ts`) that loads each user's top 10 ranked facts from the database and injects them into the AI's system prompt. The supervisor monitors user signals, chat history, and ranked facts (organized by category: industry, place, subject, preference, general) to provide personalized suggestions based on actual user data rather than generic knowledge. Facts receive recency boosts to prioritize recent user preferences. The system prompt explicitly instructs the AI to check these facts before suggesting locations or industries, preventing generic suggestions (e.g., "Kent or Somerset") that don't align with the user's actual context. Both Standard and MEGA modes share this context layer via `buildSessionContext` for consistent personalization.
