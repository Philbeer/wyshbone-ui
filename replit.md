...
- **User Results View (`UserResultsView`):** Standalone, user-facing component (`client/src/components/results/UserResultsView.tsx`) that renders run outcomes from a single `delivery_summary` object. Shows exact matches (with name/location/phone/website), closest matches (with soft_violations), shortfall banner (neutral, no apology), and suggested next question (not auto-triggered). No Tower, plan versions, percentages, artefact tabs, evidence links, or debug language. Faithful translation layer: no inference, no computed counts, no rewording. Reusable in chat, modal, or page context.
