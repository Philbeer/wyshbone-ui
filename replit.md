...
- Orphaned Run Recovery: On page load (after history loads), a recovery effect fetches recent `/api/afr/activities`, finds completed runs for the current conversation that don't have result bubbles rendered, fetches their artefacts, and creates final result messages. This handles runs that completed while the page was refreshing or after a vite HMR restart. Also replaces any lingering provisional bubbles and persists the recovered result via `persistStructuredResult`.
