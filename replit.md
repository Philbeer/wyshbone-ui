...
## Overview
The Wyshbone Chat Agent is an AI-powered chat assistant built with Node.js, Express, and React, integrating with OpenAI's GPT-5. Its primary purpose is to provide a clean, professional, and practical chat experience, delivering worldwide business discovery responses and robust tool integration capabilities for tasks such as global business discovery, lead enrichment, and triggering Bubble workflows. The project aims to streamline prospecting and lead management for Wyshbone across any country or region worldwide.

## User Preferences
I want the agent to focus on practical, worldwide business discovery with support for any country or region globally. I want to ensure that any contact information discovered is public and verifiable, with no guessing of private details. I prefer a workflow that prioritizes Google Places as the authoritative source for business discovery. The agent should be able to intelligently decide when to search for new venues versus using cached information and support conversational queries without triggering unnecessary searches. I want the agent to auto-detect and execute Bubble batch workflows based on natural language commands for any location worldwide. The system MUST ask for all 3 required fields (business type, position, location) - never default to CEO if position is not provided. I want a persistent country preference that users can see and change via chat commands, eliminating repetitive country clarification for searches in the same region.
...
- **Tool Integration:**
    ...
    - **Country Preference System:** Session-persistent country preference with left sidebar UI display (defaults to UK/GB). Users can change via chat commands like "set country to India" or "change country to Vietnam". When location has no country specified, system automatically applies the default country. API endpoints: `GET /api/country/preference` and `POST /api/country/preference`. Storage managed via `server/storage.ts` with getCountryPreference/setCountryPreference methods. Sidebar component (`CountrySidebar.tsx`) shows current country with code and change instructions.
    - **Job Management & Worldwide Location Coverage:** Background job system with comprehensive worldwide region support for running searches across geographic areas. Features:
        - **Intelligent Location Resolution** (`server/location-resolver.ts`): Parses natural language location inputs with 110+ city hints including UK regions (Scotland, Wales, Northern Ireland, England, Aberdeen, Glasgow, Edinburgh, etc.), Asian cities (Hanoi, Ho Chi Minh City, Jakarta, Manila, etc.), and worldwide coverage via local dictionaries and GeoNames geocoding fallback. **Explicit Country Parsing:** Handles patterns like "Tring, UK", "Melbourne, Australia", "Paris France" by extracting city and country separately with 0.8 confidence. Returns structured data: country (full name), country_code (ISO alpha-2), region filter, granularity, and confidence scores. Includes 43+ country name mappings (IN→India, GB→United Kingdom, VN→Vietnam, TH→Thailand, etc.) to ensure full country names are sent to Bubble's Google Places API integration. Preserves user's exact location input (e.g., "Scotland" stays as "Scotland", not replaced with "United Kingdom").
