...
- **User Results View (`UserResultsView`):** Standalone, user-facing component (`client/src/components/results/UserResultsView.tsx`) that renders run outcomes from a single `delivery_summary` object. Shows exact matches (with name/location/phone/website), closest matches (with soft_violations), shortfall banner (neutral, no apology), and suggested next question (not auto-triggered). Supports optional CVL artefact payloads (constraintsPayload, verificationPayload, evidencePayload) to render Constraints and Verification sections inline when present. Faithful translation layer: no inference, no computed counts, no rewording. Reusable in chat, modal, or page context.
